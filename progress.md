...
- **Connect UX:** `injected()` + default `multiInjectedProviderDiscovery` (EIP-6963) so each browser extension gets its own connector; `connect-button.tsx` lists all connectors (extensions first, WalletConnect last). WC project id optional for extension-only use.
